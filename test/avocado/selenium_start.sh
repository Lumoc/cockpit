...
docker run  -d -p 4444:4444 --name selenium-hub selenium/hub:3 2> >(grep -v 'Usage of loopback devices' >&2)
wait_curl /grid/console "Grid Console"
docker run -d --link selenium-hub:hub selenium/node-chrome:3 2> >(grep -v 'Usage of loopback devices' >&2)
wait_curl /grid/console "googlechrome"
docker run -d --link selenium-hub:hub selenium/node-firefox:3 2> >(grep -v 'Usage of loopback devices' >&2)
