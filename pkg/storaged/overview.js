    ...
    function init_overview(client, jobs) {
        ...
        function render_nfs_mounts() {
            ...
            function make_nfs_mount(entry, index) {
                var fsys_size;
                if (entry.mounted)
                    fsys_size = client.fsys_sizes.data[entry.fields[1]];
        ...
        function nfs_busy_dialog(dialog_title, description,
                                 entry, error,
                                 action_title, action) {
            ...
            function show(users) {
                var body;
                if (users)
                    body = mustache.render(nfs_users_tmpl,
            ...
            client.nfs.unmount_entry(entry)
                .fail(function (error) {
                    nfs_busy_dialog(_("Unable to unmount filesystem"),
                                    _("The filesystem can't be unmounted because there are still services and sessions that use files on it."),
                                    entry, error,
                                    _("Stop services and sessions and unmount"),
                                    function (users) {
            ...
            client.nfs.remove_entry(entry)
                .fail(function (error) {
                    nfs_busy_dialog(_("Unable to remove mount"),
                                    _("The filesystem can't be removed because there are still services and sessions that use files on it."),
                                    entry, error,
                                    _("Stop services and sessions and remove"),
                                    function (users) {
        ...
        $(client.fsys_sizes).on('changed', render_nfs_mounts);
