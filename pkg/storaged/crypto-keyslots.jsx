...
import {
    dialog_open,
    SelectOneRadio, TextInput, PassInput, Intermission
} from "./dialogx.jsx";
    ...
    return [
        ...
        PassInput("passphrase", _("Disk passphrase"),
                  { visible: vals => vals.needs_explicit_passphrase,
                    ...
                    explainer: _("Saving a new passphrase requires unlocking the disk. Please provide a current disk passphrase.")
                  })
    ];
}
...
const RemovePassphraseField = (tag, key, dev) => {
    return {
        ...
        render: (val, change) => {
            ...
            return (
                <div className="remove-passphrase" data-field={tag}>
                    <div className="large">
                        { fmt_to_array(_("Remove passphrase in $0?"), <b>{key_slot}</b>) }
                    </div>
                    <div>{ fmt_to_array(_("Passphrase removal may prevent unlocking $0."), <b>{dev}</b>) }</div>
                    <br />
                    <div className="radio">
                        <label>
                    ...
                    <div className="indented slot-warning" hidden={val !== false}>
...
const RemoveClevisField = (tag, key, dev) => {
    return {
        ...
        render: (val, change) => {
            return (
                <div data-field={tag}>
                    <div className="large">{ fmt_to_array(_("Remove $0?"), <b>{key.url}</b>) }</div>
                    <div>{ fmt_to_array(_("Keyserver removal may prevent unlocking $0."), <b>{dev}</b>) }</div>
                </div>
            );
        }
    };
...
export class CryptoKeyslots extends React.Component {
    ...
    render() {
        ...
        if (this.state.luks_version > 1 || (this.state.slots == null && this.state.slot_error == null))
            return null;
